...
void Shutdown()
{
    ...
    RenameThread("Positronium-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Positronium.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Positroniumd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePositroniumamount=<n> " + _("Keep N Positronium anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Positronium is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Positronium is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Positronium version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Positronium server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Positronium") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Positronium to complete") << "\n";
    ...
    nAnonymizePositroniumAmount = GetArg("-anonymizePositroniumamount", 0);
    if(nAnonymizePositroniumAmount > 999999) nAnonymizePositroniumAmount = 999999;
    if(nAnonymizePositroniumAmount < 2) nAnonymizePositroniumAmount = 2;
    ...
    LogPrintf("Anonymize Positronium Amount %d\n", nAnonymizePositroniumAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1PTM+1000 == (.1PTM+100)*10
       10PTM+10000 == (1PTM+1000)*10
    */

...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePositroniumAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePositroniumAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePositroniumAmount = strAnonymizePositroniumAmount.remove(strAnonymizePositroniumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePositroniumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePositroniumAmount*COIN) nMaxToAnonymize = nAnonymizePositroniumAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePositroniumAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePositroniumAmount));
        strAnonymizePositroniumAmount = strAnonymizePositroniumAmount.remove(strAnonymizePositroniumAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePositroniumAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizePositroniumAmount == 0){

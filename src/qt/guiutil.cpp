...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Positronium"))
        return false;
...
{
    // Convert Positronium:// to Positronium:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Positronium://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Positronium:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Positronium.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Positronium.lnk
...
{
    return GetAutostartDir() / "Positronium.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Positronium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Positronium\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Positronium app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Positronium app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Positronium-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Positronium-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Positronium-Qt"));

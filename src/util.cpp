...
int nAnonymizePositroniumAmount = 1000;
...
#else
    const char* pszModule = "Positronium";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Positronium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Positronium
    // Mac: ~/Library/Application Support/Positronium
    // Unix: ~/.Positronium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Positronium";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Positronium.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Positroniumd.pid"));
